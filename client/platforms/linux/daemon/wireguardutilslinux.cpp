...
bool WireguardUtilsLinux::updatePeer(const InterfaceConfig& config) {
    const QString serverPskKey = config.m_serverPskKey.trimmed();
    const QString serverIpv4AddrIn = config.m_serverIpv4AddrIn.trimmed();
    const QString serverIpv6AddrIn = config.m_serverIpv6AddrIn.trimmed();
    const QString activeEndpoint = !serverIpv4AddrIn.isEmpty()
            ? serverIpv4AddrIn
            : serverIpv6AddrIn;
    ...
    QByteArray pskKey = QByteArray::fromBase64(serverPskKey.toUtf8());

    logger.debug() << "Configuring peer" << config.m_serverPublicKey << "via" << activeEndpoint;
    ...
    if ((config.m_hopType != InterfaceConfig::MultiHopExit) &&
        (m_rtmonitor != nullptr)) {
        const IPAddress serverIpv4Endpoint(serverIpv4AddrIn);
        const IPAddress serverIpv6Endpoint(serverIpv6AddrIn);

        if (serverIpv4Endpoint.isValid()) {
            m_rtmonitor->addExclusionRoute(serverIpv4Endpoint);
        }
        if (serverIpv6Endpoint.isValid()) {
            m_rtmonitor->addExclusionRoute(serverIpv6Endpoint);
        }
    }
...
bool WireguardUtilsLinux::deletePeer(const InterfaceConfig& config) {
    ...
    if ((config.m_hopType != InterfaceConfig::MultiHopExit) &&
        ...
        if (serverIpv4Endpoint.isValid()) {
            m_rtmonitor->deleteExclusionRoute(serverIpv4Endpoint);
        }
        if (serverIpv6Endpoint.isValid()) {
            m_rtmonitor->deleteExclusionRoute(serverIpv6Endpoint);
        }
    }
