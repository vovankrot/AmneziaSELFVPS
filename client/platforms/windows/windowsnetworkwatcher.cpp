...
WindowsNetworkWatcher::~WindowsNetworkWatcher() {
  ...
  if (m_powerHwnd) {
    // Clear GWLP_USERDATA first to prevent use-after-free in PowerWndProcCallback
    SetWindowLongPtr(m_powerHwnd, GWLP_USERDATA, 0);
    DestroyWindow(m_powerHwnd);
    m_powerHwnd = nullptr;
    UnregisterClassW(L"PowerMonitorClass", GetModuleHandle(NULL));
  }
...
void WindowsNetworkWatcher::initialize() {
  ...
  m_powerHwnd = CreateWindowEx(0, className, L"Power Monitor", 0, 0, 0, 0, 0, NULL, NULL, GetModuleHandle(NULL), static_cast<LPVOID>(this));
  if (!m_powerHwnd) {
    logger.debug() << "Failed to create window in createPowerMonitorWindow.";
    return;
  }
  SetWindowLongPtr(m_powerHwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
