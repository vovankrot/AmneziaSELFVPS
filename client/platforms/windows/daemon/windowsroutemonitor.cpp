...
// Returns true for addresses that belong to the user's local / private
// network (RFC1918, CGNAT 100.64/10, link-local, IPv6 ULA/link-local).
// Such on-link routes must NOT be cloned into the VPN tunnel, otherwise the
// LAN — local gateway 192.168.x.x, NAS, printers, other PCs, the test VM —
// becomes unreachable while the VPN is up. by vovankrot
static bool isLocalNetworkPrefix(const QHostAddress& addr) {
  if (addr.isNull()) {
    return false;
  }
  if (addr.protocol() == QAbstractSocket::IPv4Protocol) {
    ...
    for (const auto& net : kPrivateV4) {
      if (addr.isInSubnet(QHostAddress(QLatin1String(net.net)), net.prefix)) {
        return true;
      }
    }
    return false;
  ...
  if (addr.protocol() == QAbstractSocket::IPv6Protocol) {
    return addr.isInSubnet(QHostAddress(QStringLiteral("fc00::")), 7) ||
           addr.isInSubnet(QHostAddress(QStringLiteral("fe80::")), 10);
  }
  return false;
}
...
void WindowsRouteMonitor::updateCapturedRoutes(int family, void* ptable) {
  ...
  for (ULONG i = 0; i < table->NumEntries; i++) {
    ...
    // Keep the local/private network reachable while connected. Cloning the
    ...
    data->Metric = 0;
    data->Protocol = static_cast<NL_ROUTE_PROTOCOL>(MIB_IPPROTO_NETMGMT);
    data->Loopback = false;
...
bool WindowsRouteMonitor::addExclusionRoute(const IPAddress& prefix) {
  if (!prefix.isValid()) {
    logger.warning() << "Skipping invalid exclusion route prefix";
    return true;
  }
...
bool WindowsRouteMonitor::deleteExclusionRoute(const IPAddress& prefix) {
  if (!prefix.isValid()) {
    logger.warning() << "Skipping invalid exclusion route removal";
    return true;
  }
