...
class WindowsRouteMonitor final : public QObject {
 ...
 public:
  ...
  // so the LAN stays reachable while the VPN is up. by vovankrot
  void setAllowLocalNetwork(bool enable) { m_allowLocalNetwork = enable; }
 ...
 private:
  ...
  // Leave local/private network routes on the physical interface (LAN access).
  bool m_allowLocalNetwork = true;
