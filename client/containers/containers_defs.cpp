...
QVector<amnezia::Proto> ContainerProps::protocolsForContainer(amnezia::DockerContainer container)
{
    ...
    case DockerContainer::AnyTls: return { Proto::AnyTls };
...
QMap<DockerContainer, QString> ContainerProps::containerDescriptions()
{
    return { { DockerContainer::OpenVpn,
             ...
             { DockerContainer::SSXray,
               QObject::tr("Shadowsocks proxy via XRay core. Lightweight, fast, and resistant to DPI detection.") },
             { DockerContainer::Hysteria2,
               QObject::tr("Hysteria 2 — QUIC/UDP-based VPN with HTTPS masquerading and Brutal congestion control. "
                           "Very fast on lossy networks; UDP-only.") },
             { DockerContainer::AnyTls,
               QObject::tr("AnyTLS — TLS-in-TLS proxy with multiplexing. Looks like ordinary HTTPS traffic, TCP-based.") } };
}
...
QMap<DockerContainer, QString> ContainerProps::containerDetailedDescriptions()
{
    return {
        ...
        { DockerContainer::Awg2,
          QObject::tr("AmneziaWG is a modern VPN protocol based on WireGuard, "
                      "combining simplified architecture with high performance across all devices. "
                                            "It adds obfuscation on top of WireGuard, but practical resistance to blocking depends on the current network environment and DPI/TSPU rules.\n"
        ...
        { DockerContainer::Socks5Proxy, QObject::tr("SOCKS5 proxy server") },
        { DockerContainer::SSXray,
          QObject::tr("Shadowsocks proxy implemented with XRay core.\n"
                      "Lightweight protocol that effectively masks VPN traffic as regular web data.\n"
                      "\nFeatures:\n"
                      "* Fast and lightweight\n"
                      "* Resistant to DPI detection\n"
                      "* Good choice when XRay VLESS is not available") },
        { DockerContainer::Hysteria2,
          QObject::tr("Hysteria 2 is a modern censorship-resistant proxy built on QUIC.\n"
                      ...
                      "* Will not work on networks that block UDP") },
        { DockerContainer::AnyTls,
          QObject::tr("AnyTLS is a TLS-in-TLS multiplexed proxy.\n"
...
{
    // Desktop-hardened: hide 100% DPI-detectable protocols
    // and legacy AWG (less obfuscation than AWG2)
    switch (c) {
...
{
    switch (container) {
    case DockerContainer::Cloak: return tr("Recommended");
    case DockerContainer::Xray: return tr("Alternative");
    case DockerContainer::Hysteria2: return tr("Fastest");
    default: return "";
    }
}
...
{
    switch (container) {
    case DockerContainer::Cloak: return tr("OpenVPN over Cloak will be installed. "
                                           "This is the most field-proven camouflage option in this fork: slower, but resistant to active probing and DPI blocking.");
    case DockerContainer::Xray: return tr("XRay VLESS + REALITY + XHTTP will be installed with randomized path and traffic padding. "
                                          "Use it when the server path is updated for the current XRay configuration.");
    case DockerContainer::Hysteria2: return tr("Hysteria 2 will be installed. QUIC/UDP transport with HTTPS masquerading — "
                                               "fastest option on stable networks that allow outbound UDP.");
    default: return "";
    }
}
...
int ContainerProps::easySetupOrder(DockerContainer container)
{
    ...
    case DockerContainer::Xray: return 2;
    case DockerContainer::Hysteria2: return 3;
    default: return 0;
    }
}
...
bool ContainerProps::supportsUserManagement(DockerContainer container)
{
    switch (container) {
    case DockerContainer::OpenVpn:
    case DockerContainer::ShadowSocks:
...
bool ContainerProps::supportsSiteSplitTunneling(DockerContainer container)
{
    switch (container) {
    case DockerContainer::Xray:
