...
public:
    ...
    ErrorCode cleanupServer(const ServerCredentials &credentials, bool removeDocker = false);
    ...
    // Run a complete shell script as a single file (supports if/fi, for/done, etc.)
    // Uploads script to /tmp, executes with bash, removes after completion.
    ...
    QJsonObject readServerVersion(const ServerCredentials &credentials);
...
private:
    ...
    ErrorCode logServerEnvironment(const ServerCredentials &credentials);
    ErrorCode verifyContainerIsRunning(const ServerCredentials &credentials, DockerContainer container);
...
public:
    ErrorCode writeServerVersion(const ServerCredentials &credentials, DockerContainer container);
...
private:
    ...
    bool m_loggedSshConnection = false;
    libssh::Client m_sshClient;
    QString m_loggedSshTarget;
signals:
