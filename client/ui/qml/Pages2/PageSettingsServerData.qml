...
PageType {
    ...
    Connections {
        ...
        function onCleanupServerFinished(finishedMessage) {
            PageController.closePage() // close deInstalling page
            PageController.showNotificationMessage(finishedMessage)
        }
    ...
    QtObject {
        ...
        property bool isVisible: root.isServerWithWriteAccess
        readonly property string title: qsTr("Full server cleanup")
        readonly property string description: qsTr("Removes all containers, firewall rules, and system settings applied by Amnezia")
        readonly property var tColor: AmneziaStyle.color.vibrantRed
        readonly property var clickedHandler: function() {
            var headerText = qsTr("Full server cleanup?")
            var descriptionText = qsTr("This will remove all Amnezia containers, revert firewall rules and system settings. This action cannot be undone.")
            var yesButtonText = qsTr("Clean up")
            var noButtonText = qsTr("Cancel")

            var yesButtonFunction = function() {
                if (ServersModel.isDefaultServerCurrentlyProcessed() && ConnectionController.isConnected) {
                    PageController.showNotificationMessage(qsTr("Cannot clean up server during active connection"))
                } else {
                    PageController.goToPage(PageEnum.PageDeinstalling)
                    InstallController.cleanupServer(false)
                }
            }
            var noButtonFunction = function() {
            }
