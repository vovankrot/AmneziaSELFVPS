...
void SettingsController::restoreAppConfigFromData(const QByteArray &data)
{
    ...
    const auto readImportedBool = [&newConfigData](const QString &key, bool fallback) {
        if (!newConfigData.contains(key)) {
        return fallback;
        }

        return newConfigData.value(key).toVariant().toString().toLower() == "true";
    };
    ...
    const bool bypassRuLegacy = readImportedBool("Conf/bypassRuSites", m_settings->bypassRuSites());
    const bool bypassRuGeoSites = readImportedBool("Conf/bypassRuGeoSites", bypassRuLegacy);
    const bool bypassRuGeoIp = readImportedBool("Conf/bypassRuGeoIp", bypassRuLegacy);
        m_sitesModel->setRouteMode(siteSplitTunnelingRouteMode);
        m_sitesModel->toggleSplitTunneling(siteSplittunnelingEnabled);
    m_sitesModel->setBypassRuGeoSites(bypassRuGeoSites);
    m_sitesModel->setBypassRuGeoIp(bypassRuGeoIp);
...
bool SettingsController::isAutoDisableLoopbackProxyEnabled()
{
    return m_settings->autoDisableLoopbackProxyOnConnect();
}

void SettingsController::toggleAutoDisableLoopbackProxy(bool enable)
{
    m_settings->setAutoDisableLoopbackProxyOnConnect(enable);
}
...
bool SettingsController::isAdvancedMode()
{
    return m_settings->isAdvancedMode();
}

void SettingsController::setAdvancedMode(bool enabled)
{
    m_settings->setAdvancedMode(enabled);
...
bool SettingsController::isAutoFailoverEnabled()
{
    return m_settings->isAutoFailoverEnabled();
}

void SettingsController::toggleAutoFailover(bool enable)
{
    if (m_settings->isAutoFailoverEnabled() != enable) {
        m_settings->setAutoFailoverEnabled(enable);
        emit autoFailoverChanged(enable);
    }
}
