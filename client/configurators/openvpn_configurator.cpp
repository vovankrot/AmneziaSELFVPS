                                                            ...
                                                            QString &protocolConfigString)
{
    ...
    if (!isApiConfig) {
        ...
        if (!m_settings->isSitesSplitTunnelingEnabled()) {
            ...
            // No redirect-gateway - only specific sites go through VPN
        } else if (m_settings->routeMode() == Settings::VpnAllExceptSites) {
            // All traffic through VPN, excluded sites will be routed via OS-level exclusion routes
...
{
    QString script_import = QString("sudo docker exec %1 bash -c \"cd /opt/amnezia/openvpn && "
                                    "easyrsa import-req %2/%3.req %3\"")
                                    .arg(ContainerProps::containerToString(container))
                                    .arg(amnezia::protocols::openvpn::clientsDirPath)
                                    .arg(clientId);

    QString script_sign = QString("sudo docker exec %1 bash -c \"export EASYRSA_BATCH=1; cd /opt/amnezia/openvpn && "
