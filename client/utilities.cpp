...
QString Utils::hysteriaPath()
{
#ifdef Q_OS_WIN
    return Utils::executable("hysteria/hysteria", true);
#elif defined Q_OS_LINUX
    return Utils::executable("../../client/bin/hysteria", true);
#else
    return Utils::executable("/hysteria", true);
#endif
}

QString Utils::anytlsPath()
{
#ifdef Q_OS_WIN
    return Utils::executable("anytls/anytls-client", true);
#elif defined Q_OS_LINUX
    return Utils::executable("../../client/bin/anytls-client", true);
#else
    return Utils::executable("/anytls-client", true);
#endif
}

namespace {
#ifdef Q_OS_WIN
const QString kInternetSettingsKey =
    "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings";
#endif
}

QString Utils::getActiveSystemProxy()
{
#ifdef Q_OS_WIN
    QSettings settings(kInternetSettingsKey, QSettings::NativeFormat);
    const bool enabled = settings.value("ProxyEnable", 0).toInt() != 0;
    if (!enabled) {
        return {};
    }
    return settings.value("ProxyServer").toString().trimmed();
#else
    return {};
#endif
}

bool Utils::isLoopbackSystemProxyActive(QString *outServer)
{
    const QString server = getActiveSystemProxy();
    if (server.isEmpty()) {
        return false;
    }

    // ProxyServer may be "host:port" or "scheme=host:port;scheme=host:port".
    const bool loopback = server.contains("127.0.0.1")
        || server.contains("localhost", Qt::CaseInsensitive)
        || server.contains("[::1]");

    if (loopback && outServer) {
        *outServer = server;
    }
    return loopback;
}

bool Utils::disableSystemProxy()
{
#ifdef Q_OS_WIN
    QSettings settings(kInternetSettingsKey, QSettings::NativeFormat);
    ...
    // Notify running applications that proxy settings changed. Resolve the
    // Wininet symbols at runtime to avoid a static link dependency.
    using InternetSetOptionW_t = BOOL(WINAPI *)(LPVOID, DWORD, LPVOID, DWORD);
    HMODULE wininet = LoadLibraryW(L"wininet.dll");
    if (wininet) {
        auto pInternetSetOption =
            reinterpret_cast<InternetSetOptionW_t>(GetProcAddress(wininet, "InternetSetOptionW"));
        if (pInternetSetOption) {
            const DWORD kInternetOptionSettingsChanged = 39;
            const DWORD kInternetOptionRefresh = 37;
            pInternetSetOption(nullptr, kInternetOptionSettingsChanged, nullptr, 0);
            pInternetSetOption(nullptr, kInternetOptionRefresh, nullptr, 0);
        }
        FreeLibrary(wininet);
    }
    return true;
